...
function Connection(options) {
    ...
    this.offset = 0;
    this.correlationCache = cache({ ttl: 30, interval: 50 });
 ...
 */
Connection.prototype.send = function (correlationId, data, noresponse, label = 'undefined') {
    ...
    function _send() {
        return new Promise(function (resolve, reject) {
            ...
            self.correlationCache.set(correlationId, { label, data, noresponse });
...
Connection.prototype._receive = function (data) {
    ...
    if (!this.queue.hasOwnProperty(correlationId)) {
        console.error('Wrong correlationId received:' + correlationId + ', referred by: ' + JSON.stringify(this.correlationCache.get(correlationId)));
    } else {
